...
class MyRolloutBuffer(BaseBuffer):
    ...
    def reset(self) -> None:
        ...
        self.actions_eps = th.zeros((self.buffer_size, self.n_envs, self.action_dim), dtype=th.float64, device=self.device)
    ...
    def add(
        ...
        :param log_prob: log probability of the action
            following the current policy.
        :param action_eps: noise vector used to compute actions
        """
        ...
        action_eps = action_eps.reshape((self.n_envs, self.action_dim))

        self.observations[self.pos] = np.array(obs)
        self.actions[self.pos] = np.array(action)
        self.actions_eps[self.pos] = action_eps
